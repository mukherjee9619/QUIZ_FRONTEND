...
export default function Quiz() {
  ...
  useEffect(() => {
    fetch(
      `https://quiz-backend-gamma.vercel.app/api/questions-public?subjectId=${subjectId}`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    fetch(`https://quiz-backend-gamma.vercel.app/api/admin/subjects/${subjectId}`)
      .then((res) => res.json())
      .then((data) => {
        setSubjectName(data.displayName || data.name);
      })
      .catch(() => {
  ...
  const handleSubmit = async (force = false) => {
    ...
    const timeTakenSec = Math.ceil(questions.length * 0.7 * 60) - timeLeft;
    ...
    try {
      await fetch("https://quiz-backend-gamma.vercel.app/api/result/email", {
        method: "POST",
